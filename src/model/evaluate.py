...
def main():
    ...
    if files:
        for win_index in range(start_window, start_window + num_windows):
            ...
            model_fp = os.path.join(model_dir, f'{model_id}_model.pt')
            ...
            evaluate(model, test_dl, device, lai_pred, lai_true, lai_prob, lai_inds)
            elapsed_t = time.strftime('%H:%M:%S', time.gmtime(time.time() - start_t))
    ...
    if smoother == 'simple' or files:
        lai_pred, lai_true = simple_smooth(lai_true, lai_prob, kernel_radius=10)
    elif smoother == 'cnn':
        lai_pred, lai_true = cnn_smooth(data_dir, model_dir, device, num_classes)

    with open(f'{model_type}_{case_str}.txt', 'w') as out_f:
        for ind_idx in lai_pred.keys():
            out_f.write(f'{sum(1 for x, y in zip(lai_pred[ind_idx], lai_true[ind_idx]) if x == y) / len(lai_pred[ind_idx])}\n')

    # Plot LAI confusion matrix
    if plot_conf:
        lai_true_conf = []
        lai_pred_conf = []
        for key in lai_pred.keys():
            lai_true_conf.extend(lai_true[key])
            lai_pred_conf.extend(lai_pred[key])
        sn.set(font_scale=2.0)
        lai_classes = ['ABCDEFGHIJKL'[i] for i in range(num_classes)]
        lai_conf = confusion_matrix(lai_true_conf, lai_pred_conf).astype('float')
        ...
        for x, y in zip(lai_true_conf, lai_pred_conf):
            acc_dic[x].append(1 if x == y else 0)

        overall_hits = 0
        overall_len  = 0
        for key, val in acc_dic.items():
            overall_hits += sum(val)
            overall_len  += len(val)
            print(f'Overall LAI Acc for population {key} = {sum(val) / len(val) * 100}%')
        print(f'Overall LAI Acc = {(overall_hits / overall_len) * 100}%')

def cnn_smooth(data_dir, model_dir, device, num_classes):
    smooth_pred = []
    smooth_true = []
    test_dl = smooth_loader(data_dir)

    model_id = f'smoother'
    model_fp = os.path.join(model_dir, 'smoother', f'{model_id}_model.pt')
    ...
    with torch.no_grad():
        for (labels, probs, cms) in test_dl:
            ...
            for i, (preds, truths) in enumerate(zip(smooth_class, labels.tolist())):
...
def simple_smooth(truth_mat, probs_mat, kernel_radius=2):
    smooth_pred  = defaultdict(list)
    smooth_truth = defaultdict(list)

    for win_idx in range(len(probs_mat)):
        left_lim  = max(0, win_idx - kernel_radius)
        right_lim = min(len(probs_mat) - 1, win_idx + kernel_radius) + 1

        for ind_idx in range(len(probs_mat[win_idx])):
            probs_raw = probs_mat[left_lim:right_lim,ind_idx]
            preds = []
            probs = []

            for p in probs_raw:
                probs.extend(p)
                preds.extend([i for i in range(len(p))])

            smooth = int(weighted_mode(preds, probs)[0][0])
            smooth_pred[ind_idx].append(smooth)
            smooth_truth[ind_idx].append(truth_mat[win_idx][ind_idx])
...
def evaluate(model, test_i, device, lai_pred, lai_true, lai_prob, lai_inds):
    ...
    model.eval()
    with torch.no_grad():
        for (pop_lab, snp, aug, ind_idx) in test_i:
            snp     = snp.to(device)
            aug     = aug.to(device)
...
def handle_args():
    ...
    parser.add_argument('--case',
        help='Experiment ID string', type=str)
    ...
    parser.add_argument('--smoother', required=False,
        help='Smoother to use (simple / cnn)', default='simple', type=str)
    parser.add_argument('--files',
        help='Write intermediate files for CNN smoother training', action=argparse.BooleanOptionalAction)
