...
from model import BLSTM, Multilayer, Transformer, CNN
...
def main():
  ...
  train_dl, valid_dl, window_size = data_management.create_training_loaders(
    data_dir, train_fn, valid_fn, batch_size=batch_size)
  elapsed_t = time.strftime('%H:%M:%S', time.gmtime(time.time() - start_t))
  print(f'Done creating data loaders, elapsed time = {elapsed_t}.')

  # Set model to train
  model_str = f'{model_type}_{win_index}_{learning_r}_{optim_type}'
  ...
  train(model, optimizer, train_dl, valid_dl, len(train_dl) // 3, save_dir,
...
def handle_args():
  ...
  parser.add_argument('-e','--epochs',
    help='Epochs to train for', default=10, type=int)
