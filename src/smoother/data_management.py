...
class WindowDataset(Dataset):
    ...
    def __init__(self, predictions_fp, truths_fp, cms_fp, valid=False, transform=None, target_transform=None):
        self.predictions = np.transpose(np.load(predictions_fp).astype(np.float32), (1, 0, 2))[:, :128, :]
        self.truths = np.transpose(np.load(truths_fp).astype(int), (1, 0))[:, :128]
        self.cms = np.load(cms_fp).astype(np.float32)[:128]
        self.valid = valid

        if self.valid:
            self.length = int(len(self.predictions) * .1)
        else:
            self.length = int(len(self.predictions) * .9)
    ...
    def __getitem__(self, idx):
        if self.valid:
            idx += int(len(self.predictions) * .9)
        return self.truths[idx], self.predictions[idx], self.cms


def create_training_loaders(data_dir, batch_size=32):
...
def create_testing_loader(data_dir, batch_size=32):
